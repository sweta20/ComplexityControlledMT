...
if [[ $newsela_eq == True ]]; then
	if [[ $grade_eq == True ]]; then
		...
		cat $data_dir/train_esen_eq.$type_1 >> $sub_data_dir/train.src.tmp
		...
		cat $data_dir/dev_esen_eq.$type_1 >> $sub_data_dir/dev.src.tmp
		...
		cat $data_dir/train_esen_eq.$type_1 |  sed 's/^<\w*>\t\ *//'   >> $sub_data_dir/train.src.tmp
		...
		cat $data_dir/dev_esen_eq.$type_1 |  sed 's/^<\w*>\t\ *//' >> $sub_data_dir/dev.src.tmp
...
if [[ $newsela_neq == True ]]; then
	if [[ $grade_neq == True ]]; then
		...
		cat $data_dir/train_esen_neq.$type_1 >> $sub_data_dir/train.src.tmp
		...
		cat $data_dir/dev_esen_neq.$type_1 >> $sub_data_dir/dev.src.tmp
		...
		cat $data_dir/train_esen_neq.$type_1 |  sed 's/^<\w*>\t\ *//'   >> $sub_data_dir/train.src.tmp
		...
		cat $data_dir/dev_esen_neq.$type_1 | sed 's/^<\w*>\t\ *//' >> $sub_data_dir/dev.src.tmp
